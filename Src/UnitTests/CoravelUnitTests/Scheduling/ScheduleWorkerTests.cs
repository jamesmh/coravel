...
public class ScheduleWorkerTests
{
    ...
    public async Task ScheduleWorkerSuccessfulWithOneThreadPerWorker()
    {
        for (int i = 0; i < 100; i++)
        {
            ...
            List<int> threads1 = new();
            List<int> threads2 = new();
            List<int> threads3 = new();
            List<int> threads4 = new();
            List<int> threads5 = new();
            List<int> threads6 = new();
            List<int> threads7 = new();
            List<int> threads8 = new();
    ...
    public async Task ScheduleWorkerSuccessfulOnMultipleThreadsWithMutiRuns()
    {
        ...
        await Task.WhenAll(
            scheduler.RunAtAsync(DateTime.Parse("2018/06/07", new CultureInfo("en-US"))),
            scheduler.RunAtAsync(DateTime.Parse("2018/06/07", new CultureInfo("en-US"))),
            scheduler.RunAtAsync(DateTime.Parse("2018/06/07", new CultureInfo("en-US"))),
            scheduler.RunAtAsync(DateTime.Parse("2018/06/07", new CultureInfo("en-US")))
        );
    ...
    public async Task ScheduleWorkerSuccessfulWithDifferentSchedules()
    {
        ...
        await scheduler.RunAtAsync(DateTime.Parse("2018/06/07 12:05 am", new CultureInfo("en-US")));
